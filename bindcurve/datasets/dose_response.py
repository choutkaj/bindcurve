...
class CompoundData:
    ...
    def aggregate_replicates(
        ...
        *,
        ...
        Returns a table with one row per concentration containing the aggregate
        response and point-level replicate diagnostics.
        """
...
class DoseResponseData:
    ...
    The canonical internal schema is long-form/tidy data with required columns
    ``compound_id``, ``concentration``, and ``response``. Missing
    ``experiment_id`` and ``replicate_id`` columns are filled with default values.
    """
